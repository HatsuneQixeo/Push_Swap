...
static char	*ft_unsigned(uintptr_t hex, t_flags *flags)
{
	...
	if (flags->format == 'p')
		...
		str = ft_utoa_base((unsigned int)hex, "0123456789abcdef");
	else if (flags->format == 'X')
		str = ft_utoa_base((unsigned int)hex, "0123456789ABCDEF");
	else if (flags->format == 'o')
		str = ft_utoa_base((unsigned int)hex, "01234567");
	else if (flags->format == 'u')
		return (ft_utoa_base((unsigned int)hex, "0123456789"));
	else
		ft_dprintf(2, "Unknown character in ft_unsigned: %c\n", flags->format);
	if (flags->format == 'p')
		return (ft_strmodify(str, "0x", ft_strrjoin));
	else if (*str == '0' || !flags->sharp)
		return (str);
	else if (flags->format == 'x')
		return (ft_strmodify(str, "0x", ft_strrjoin));
	else if (flags->format == 'X')
...
int	ft_conversion(va_list args, t_flags *flags)
{
	...
	else if (flags->format == 'p' || flags->format == 'u'
		|| flags->format == 'o'
